...
mcoind tx revenue register $CONTRACT $NONCE $WITHDRAWER \
...
```bash
# Check revenues
mcoind q revenue contract $CONTRACT
```
...
mcoind tx revenue update $CONTRACT $WITHDRAWER \
...
mcoind tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000pose \
--from=mm
```
