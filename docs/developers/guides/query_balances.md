...
- [`mcoind` & Tendermint RPC](#mcoind--tendermint-rpc)
- [JSON-RPC](#json-rpc)
- [gRPC](#grpc)
...
## `mcoind` & Tendermint RPC

Upon [installation](../../validators/quickstart/installation.md) and [configuration](../../validators/quickstart/binary.md) of the Evmos Daemon, developers can query account balances using `mcoind` with the following CLI command:

```bash
$ mcoind query bank balances $EVMOSADDRESS --count-total=$COUNTTOTAL --height=$HEIGHT --output=$OUTPUT --node=$NODE
balances:
