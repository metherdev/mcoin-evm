...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`mcoind`](../../validators/quickstart/binary.md), use the following command with `register-erc20`:

```bash
mcoind tx gov submit-proposal register-erc20 ERC20_ADDRESS...\
  --title=<title> \
...
```bash
mcoind tx gov submit-proposal register-erc20 --proposal=<path/to/proposal.json>
```
