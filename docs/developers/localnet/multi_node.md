...
calling the `mcoind testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/mcoind` directory in each container.
...
```bash
# node 0: daemon logs
docker exec composednode0 tail mcoind.log
...
To interact with `mcoind` and start querying state or creating txs, you use the
`mcoind` directory of any given node as your `home`, for example:

```bash
mcoind keys list --home ./build/node0/mcoind
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/mcoind/key_seed.json` and can be restored to the CLI using the `mcoind keys add --restore` command
:::
