...
```bash
mcoind init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .mcoind/config folder

```bash
mv $HOME/genesis.json ~/.mcoind/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.mcoind/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.mcoind/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.mcoind/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.mcoind/config/config.toml
```

### Create mcoind service

```bash
echo "[Unit]
Description=mcoind Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which mcoind) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/mcoind.service; sudo mv $HOME/mcoind.service /etc/systemd/system/
```

```bash
sudo systemctl enable mcoind.service && sudo systemctl daemon-reload
```

### Run mcoind

```bash
sytemctl start mcoind
```

### Check logs

```bash
journalctl -u mcoind -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.mcoind/config/config.toml
```
