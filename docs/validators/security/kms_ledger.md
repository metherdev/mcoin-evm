...
- Edit `addr` to point to your `mcoind` instance.
- Adjust `chain-id` to match your `.mcoind/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `mcoind`.
...
The KMS may complain that it cannot connect to `mcoind`. That is fine, we will fix it in the next section.
...
You need to enable KMS access by editing `.mcoind/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `mcoind`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell mcoind the key that we've got in the previous section.

```bash
mcoind gentx --name kmsval --pubkey <pub_key>
```

Now start `mcoind`. You should see that the KMS connects and receives a signature request.
