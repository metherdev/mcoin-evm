...
echo "export DAEMON_NAME=mcoind" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.mcoind" >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for `cosmosvisor` in your `DAEMON_HOME` directory (`~/.mcoind`) and copy over the current binary.

```bash
mkdir -p ~/.mcoind/cosmovisor
mkdir -p ~/.mcoind/cosmovisor/genesis
mkdir -p ~/.mcoind/cosmovisor/genesis/bin
mkdir -p ~/.mcoind/cosmovisor/upgrades

cp $GOPATH/bin/mcoind ~/.mcoind/cosmovisor/genesis/bin
```

To check that you did this correctly, ensure your versions of `cosmovisor` and `mcoind` are the same:
...
```bash
sudo tee /etc/systemd/system/mcoind.service > /dev/null <<EOF
[Unit]
...
Environment="DAEMON_HOME=$HOME/.mcoind"
...
sudo -S systemctl enable mcoind
sudo -S systemctl start mcoind
```

You can check the status with:

```bash
systemctl status mcoind
```
