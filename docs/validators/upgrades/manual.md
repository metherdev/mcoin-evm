...
Before upgrading the Evmos version. Stop your instance of `mcoind` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `mcoind` is running.
:::
...
```bash
rm $HOME/.mcoind/config/addrbook.json
