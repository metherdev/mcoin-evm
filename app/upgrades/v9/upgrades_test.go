...
func (suite *UpgradeTestSuite) TestReturnFundsFromCommunityPool() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("mcoin", res))
	...
	suite.Require().Equal(balanceBefore.AmountOf("mcoin"), sdk.NewDecFromInt(res))
	...
	for i := range v9.Accounts {
		...
		balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "mcoin")
