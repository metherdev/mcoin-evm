...
Find below a list of `mcoind` commands added with the `x/vesting` module. You can obtain the full list by using the `mcoind -h` command.
...
```go
mcoind add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
mcoind query vesting balances ADDRESS [flags]
```
...
```go
mcoind tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
mcoind tx vesting clawback ADDRESS [flags]
```
...
```go
mcoind tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
